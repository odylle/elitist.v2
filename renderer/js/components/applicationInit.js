...
const init = async () => {
  return new Promise(async (resolve) => {
    ...
    const { readFolder, readJournal, orderLogFiles, logDateToTimestamp } =
      require("../modules/journal").functions;
    logContainer.insertBefore(
      logEntry(
        ["fa", "fa-folder-open"],
        "check folder for unprocessed logfiles",
      ),
      logContainer.firstChild,
    );
    await readFolder(folder)
      .then(async (files) => {
        let logFiles = await files.filter(
          (file) => file.split(".").pop() == "log",
        );
        logContainer.insertBefore(
          logEntry(["fa", "fa-filter"], "filter out non logfiles"),
          logContainer.firstChild,
        );
        return logFiles;
      })
      .then(async (logFiles) => {
        logContainer.insertBefore(
          logEntry(["fa", "fa-sort"], "ordering logfiles by date"),
          logContainer.firstChild,
        );
      ...
      .then(async (orderedLogFiles) => {
        ...
        for (let file of orderedLogFiles) {
          ...
          if (!processedLogFiles.includes(logDate)) {
            logContainer.insertBefore(
              logEntry(["fa", "fa-book-open"], `Reading: ${file}`),
              logContainer.firstChild,
            );
            ...
            await readJournal(folder + "/" + file).then(async (result) => {
              await db.bodies.count().then((count) => {
                logContainer.insertBefore(
                  logEntry(
                    ["fa", "fa-plus", "info"],
                    `<span class="highlight">${count - bodiesInDB}</span> Bodies added`,
                  ),
                  logContainer.firstChild,
                );
                bodiesInDB = count;
              ...
              logContainer.insertBefore(
                logEntry(
                  ["fa", "fa-check", "success"],
                  `File read: <span class="highlight">${result}</span> lines processed`,
                ),
                logContainer.firstChild,
              );
            });
      ...
      .catch((e) => {
        console.log(folder);
        logContainer.insertBefore(
          logEntry(["fa", "fa-exclamation-triangle", "error"], e),
          logContainer.firstChild,
        );
      });
    logContainer.insertBefore(
      logEntry(
        ["fa", "fa-check-square", "success"],
        `Files read, events processed, loading application...`,
      ),
      logContainer.firstChild,
    );
    resolve();
