...
const bodySummary = async (address) => {
  ...
  starsElement =
    stars.length > 0
      ? `<span><i class="fa icarus-terminal-star"></i> ${stars.length} stars</span>`
      : "";
  planetsElement =
    planets.length > 0
...
const systemDetails = (system) => {
  ...
  if (system.population) {
    details.push(
      `<div>Population<span>${formatNumber(parseFloat(system.population))}</span></div>`,
    );
  }
  if (system.allegiance) {
    details.push(
      `<div>Allegiance<span class="allegiance-${system.allegiance.toLowerCase()}">${system.allegiance}</span></div>`,
    );
  }
  if (system.economy && system.economy.first != "None") {
    let second = system.economy.second
      ? `<span class="secondary">${system.economy.second}</span>`
      : "";
    details.push(
      `<div>Economy<span>${system.economy.first}</span>${second}</div>`,
    );
  }
  ...
  if (system.security) {
    ...
    details.push(
      `<div>Security<span class="security-${securityCss}">${system.security}</span></div>`,
    );
  }
  return details.join("");
};
...
const getPlanetIcon = (planetClass) => {
  ...
  let map = {
    ...
    "Gas giant with water based life": {
      icon: "planet-water-based-life",
      css: "water-based-life",
    },
    "Gas giant with ammonia based life": {
      icon: "planet-ammonia-based-life",
      css: "ammonia-based-life",
    },
  ...
  return map[planetClass]
    ? iconTemplate(map[planetClass].icon, map[planetClass].css)
    : iconTemplate("planet", "empty");
};
...
const renderBodyElement = async (address, body) => {
  const {
    lineFormatter,
    formatNumber,
  } = require("../../modules/journal/functions");
...
const renderSignals = async (address, body) => {
  let result = "";
  if (
    address in store.get(`session.exobiology.signals`) &&
    body in store.get(`session.exobiology.signals.${address}`)
  ) {
