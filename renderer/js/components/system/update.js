...
const updateBodies = async (id) => {
    ...
    let body = await db.bodies.where({"address": store.get("session.location.address"), "id": id}).first()
    if (document.getElementById(store.get("session.location.address") + "."+ id)) {
        // Set Star Class if unknown
        const { starClassification } = require("./index.js")
        let bodyElement = document.getElementById(store.get("session.location.address") + "."+ id)
        if (body.type == "Star") {
            bodyElement.querySelector(".class").innerHTML = starClassification(body.class)
        }
    } else {
        const { renderBodies, summary } = require("./index.js")
        let bodyElement = await renderBodies(store.get("session.location.address"), body)
        await bodiesElement.appendChild(bodyElement)
        system.querySelector(".summary").innerHTML = await summary(store.get("session.location.address"))
    }
