  ...
  return new Promise(async (resolve) => {
    line.Signals.forEach((signal) => {
      if (signal.Type_Localised == "Biological") {
        if (!store.get("session.exobiology.signals." + line.SystemAddress)) {
          store.set("session.exobiology.signals." + line.SystemAddress, {});
        }
        store.set(
          "session.exobiology.signals." +
            line.SystemAddress +
            "." +
            line.BodyID,
          signal.Count,
...
const SAASignalsFound = async (line) => {
  ...
  return new Promise(async (resolve) => {
    ...
    if ("Genuses" in line) {
      ...
      store.set(
        "session.exobiology.signals." + line.SystemAddress + "." + line.BodyID,
        genuses,
      );
...
const ScanOrganic = async (line) => {
  return new Promise(async (resolve) => {
    let genus = line.Genus_Localised,
      species = line.Species_Localised.split(" ").pop(),
      variant = line.Variant_Localised.split(" ").pop();
    if (line.ScanType == "Log") {
      if (!store.get("session.exobiology.signals." + line.SystemAddress)) {
        store.set("session.exobiology.signals." + line.SystemAddress, {});
      }
      store.set(
        `session.exobiology.signals.${line.SystemAddress}.${line.Body}.${genus}.species`,
        species,
      );
      store.set(
        `session.exobiology.signals.${line.SystemAddress}.${line.Body}.${genus}.variant`,
        variant,
      );
      store.set(
    ...
    if (line.ScanType == "Analyse") {
      ...
      if (
        ...
        store.set(
          `session.exobiology.samples.${genus}.${species}.${variant}.count`,
          1,
        );
      } else {
        let count = store.get(
...
const SellOrganicData = async (line) => {
  return new Promise(async (resolve) => {
    ...
    for await (let organic of line.BioData) {
      let obj = new Organic(
        organic.Genus_Localised,
        organic.Species_Localised.split(" ").pop(),
        organic.Variant_Localised.split(" ").pop(),
      );
      await obj.init();
